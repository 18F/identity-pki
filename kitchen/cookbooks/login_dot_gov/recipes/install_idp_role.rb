...
ruby_block 'add_idp_role' do
  block do
    node.run_list.delete('recipe[login_dot_gov::install_idp_role]')
    node.run_list << 'role[idp]'
  end
  only_if { node[:recipes].include?('login_dot_gov::install_idp_role') }
end
