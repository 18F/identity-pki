...
resource "aws_acm_certificate" "gitlab-pages" {
  ...
  tags = {
    Name = "gitlabpages${var.env_name}.${var.root_domain}"
  }
}

resource "aws_route53_record" "gitlab-pages-validation" {
  for_each = {
    for dvo in aws_acm_certificate.gitlab-pages.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "gitlab-pages" {
  certificate_arn         = aws_acm_certificate.gitlab-pages.arn
  validation_record_fqdns = [for record in aws_route53_record.gitlab-pages-validation : record.fqdn]
}
