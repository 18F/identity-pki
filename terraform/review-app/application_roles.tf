module "application_iam_roles" {
  ...
  env_name                       = var.cluster_name # Has to be alphanumeric, otherwise a couple bucket Sid's will fail because of the name containing a _
  region                         = var.region
  apps_enabled                   = 1
  ...
  pivcac_route53_zone_id         = data.aws_route53_zone.pivcac.zone_id
