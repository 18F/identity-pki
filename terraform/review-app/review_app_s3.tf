...
resource "aws_s3_bucket" "pivcac_cert_bucket" {
  bucket = "login-gov-pivcac-${var.cluster_name}.${data.aws_caller_identity.current.account_id}-${var.region}"

  tags = {
    Name = "login-gov-pivcac-${var.cluster_name}.${data.aws_caller_identity.current.account_id}-${var.region}"
  }
}
...
resource "aws_s3_bucket" "pivcac_public_cert_bucket" {
  bucket = "login-gov-pivcac-public-cert-${var.cluster_name}.${data.aws_caller_identity.current.account_id}-${var.region}"

  tags = {
    Name = "login-gov-pivcac-public-cert-${var.cluster_name}.${data.aws_caller_identity.current.account_id}-${var.region}"
  }
}
