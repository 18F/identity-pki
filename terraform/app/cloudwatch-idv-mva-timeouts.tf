  ...
  "widgets": [
    {
      ...
      "type": "log",
      "properties": {
        "query": "SOURCE '${var.env_name}_/srv/idp/shared/log/events.log' | filter name = 'IdV: doc auth verify proofing results' and properties.event_properties.proofing_results.context.stages.state_id.vendor_name = 'aamva:state_id' and ispresent(properties.event_properties.proofing_results.context.stages.state_id.exception)\n| stats count(*) as error_count by bin(6hr)",
        "region": "${var.region}",
        "stacked": false,
    ...
    },
    {
      ...
      "type": "log",
      "properties": {
        "query": "SOURCE '${var.env_name}_/srv/idp/shared/log/events.log' | filter name = 'IdV: doc auth verify proofing results' and properties.event_properties.proofing_results.context.stages.state_id.vendor_name = 'aamva:state_id'\n| fields ispresent(properties.event_properties.proofing_results.context.stages.state_id.exception) as @exception\n| stats sum(@exception) / count(*) * 100 as error_rate by bin(6hr)",
        "region": "${var.region}",
        "stacked": false,
