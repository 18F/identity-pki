    ...
    "widgets": [
        ...
        },
        {
            ...
            "properties": {
                "metrics": [
                    %{if var.idp_use_rds~}
                    [ "AWS/RDS", "DiskQueueDepth", "DBInstanceIdentifier", "${aws_db_instance.idp[0].id}", { "label": "Database (RDS)" } ],
                    %{endif~}
        ...
        },
        {
            ...
            "properties": {
                "metrics": [
                    %{if var.idp_use_rds~}
                    [ "AWS/RDS", "FreeableMemory", "DBInstanceIdentifier", "${aws_db_instance.idp[0].id}", { "label": "Database (RDS)" } ],
                    %{endif~}
                    %{if var.idp_aurora_enabled && var.idp_cluster_instances >= 2~}
        ...
        },
        {
                ...
                "properties": {
                    "metrics": [
                        %{if var.idp_use_rds~}
                        [ "AWS/RDS", "ReadLatency", "DBInstanceIdentifier", "${aws_db_instance.idp[0].id}", { "label": "Database (RDS)" } ],
                        %{endif~}
                        %{if var.idp_aurora_enabled && var.idp_cluster_instances >= 2~}
        ...
        },
        {
            ...
            "properties": {
                "metrics": [
                    %{if var.idp_use_rds~}
                    [ "AWS/RDS", "NetworkTransmitThroughput", "DBInstanceIdentifier", "${aws_db_instance.idp[0].id}", { "label": "Database (RDS)" } ],
                    %{endif~}
                    %{if var.idp_aurora_enabled && var.idp_cluster_instances >= 2~}
