...
# in your data-warehouse/<ENV>/main.tf file
...
module "gitlab" {
  ...
  endpoint_subnet_ids     = slice([for subnet in aws_subnet.apps : subnet.id], 0, 2)
  vpc_id                  = aws_vpc.analytics_vpc.id
  name                    = var.name
  env_name                = var.env_name
  allowed_security_groups = [aws_security_group.base.id]
  route53_zone_id         = aws_route53_zone.internal.id
...
# data-warehouse/<ENV>/main.tf file, as well as the `gitlab_enabled`
...
module "env-runner" {
  ...
  aws_vpc                          = aws_vpc.analytics_vpc.id
  ...
  gitlab_runner_pool_name          = "analytics-env-runner"
  instance_type_gitlab_runner      = var.instance_type_env_runner
  proxy_server                     = "obproxy-analytics-env-runner.login.gov.internal"
  root_domain                      = var.root_domain
  route53_id                       = ""
  route53_internal_zone_id         = aws_route53_zone.internal.id
  runner_subnet_ids                = [for subnet in aws_subnet.apps : subnet.id]
