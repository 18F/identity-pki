...
data "aws_iam_policy_document" "guard_duty_threat_feed_access" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::${aws_s3_bucket.guard_duty_threat_feed_s3_bucket.id}"
    ]
  }
  statement {
  ...
  statement {
    ...
    resources = [
      "${aws_ssm_parameter.guard_duty_threat_feed_public_key.arn}",
      "${aws_ssm_parameter.guard_duty_threat_feed_private_key.arn}"
    ]
  }
}

resource "aws_iam_policy" "guard_duty_threat_feed_access" {
  name        = "${var.guard_duty_threat_feed_name}-lambda-policy"
  description = "Policy for ${var.guard_duty_threat_feed_name}-lambda access"
  policy      = data.aws_iam_policy_document.guard_duty_threat_feed_access.json
}

resource "aws_iam_role_policy_attachment" "guard_duty_threat_feed_basic_lambda" {
  role       = aws_iam_role.guard_duty_threat_feed_lambda_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy_attachment" "guard_duty_threat_feed_access_policies" {
  role       = aws_iam_role.guard_duty_threat_feed_lambda_role.name
  policy_arn = aws_iam_policy.guard_duty_threat_feed_access.arn
}
