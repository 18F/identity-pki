...
output "override_cidrs_v4" {
  description = "IPv4 CIDR blocks to allow through the WAFv2 web ACL(s)."
  value = length(var.override_cidr_blocks_v4) > 0 ? sort(
    aws_wafv2_ip_set.override_ips_v4[0].addresses
  ) : []
}

output "override_cidrs_v6" {
  description = "IPv6 CIDR blocks to allow through the WAFv2 web ACL(s)."
  value = length(var.override_cidr_blocks_v6) > 0 ? sort(
    aws_wafv2_ip_set.override_ips_v6[0].addresses
  ) : []
}
