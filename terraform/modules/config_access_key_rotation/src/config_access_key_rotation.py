...
rotationDate = (
    datetime.datetime.now() - datetime.timedelta(days=rotationPeriod)
).date()
inactivationDate = (
    datetime.datetime.now() - datetime.timedelta(days=oldKeyInactivationPeriod)
).date()
deletionDate = (
...
def handle_oldest_key(user_name, recipient_email, sender_email, oldest_key):
    ...
    keys_inactivated_days_count = oldKeyInactivationPeriod - access_keys_age
