...
resource "aws_cloudwatch_log_subscription_filter" "cloudwatch_subscription_filter_s3" {
  count           = var.send_logs_to_s3
  for_each        = var.cloudwatch_log_group_name
  name            = var.cloudwatch_subscription_filter_name + "-to-s3"
...
resource "aws_cloudwatch_log_subscription_filter" "cloudwatch_subscription_filter_soc" {
  for_each        = var.cloudwatch_log_group_name
  name            = var.cloudwatch_subscription_filter_name + "-to-soc"
  log_group_name  = each.key
  filter_pattern  = each.value
