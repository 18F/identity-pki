...
resource "aws_iam_role" "idp-pinpoint-east" {
  name = "idp-pinpoint-east"
...
output "pinpoint_idp_role_arn" {
  value = aws_iam_role.idp-pinpoint-east.arn
}

# Allow sending SMS/Voice messages with Pinpoint
resource "aws_iam_role_policy" "idp-pinpoint-send-east" {
  name   = "idp-pinpoint-send-east"
  role   = aws_iam_role.idp-pinpoint-east.id
  policy = <<EOM
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role" "admin-east" {
  name = "admin-east"
