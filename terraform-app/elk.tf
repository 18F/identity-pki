...
data "aws_iam_policy_document" "cloudtrail" {
  statement {
    principals = {
      type = "AWS"
      identifiers = ["${aws_iam_role.elk_iam_role.arn}"]
    }
    actions = [
      "s3:ListBucket"
    ]
    resources = [
      "arn:aws:s3:::login-gov-${var.env_name}-cloudtrail"
    ]
  }
  statement {
    principals = {
...
resource "aws_s3_bucket" "cloudtrail" {
  bucket = "login-gov-${var.env_name}-cloudtrail"
  force_destroy = true
