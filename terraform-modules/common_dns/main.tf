...
resource "aws_route53_record" "main_dmarc" {
    ...
    ttl = "900"
    type = "TXT"
    records = ["v=DMARC1; p=reject; pct=100; fo=1; ri=3600; rua=mailto:gsalogin@rua.agari.com,mailto:dmarc-reports@login.gov,mailto:reports@dmarc.cyber.dhs.gov,mailto:dmarc_agg@vali.email; ruf=mailto:dmarc-forensics@login.gov"]
}
