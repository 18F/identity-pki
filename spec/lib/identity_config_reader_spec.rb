...
RSpec.describe IdentityConfigReader do
  ...
  let(:s3_client) { nil }
  subject(:reader) { IdentityConfigReader.new(logger: logger, s3_client: s3_client) }

  context 'in the datacenter' do
    let(:s3_client) { Aws::S3::Client.new(stub_responses: true) }
    let(:s3_contents) do
      {
        'int/pivcac/v1/application.yml' => OVERRIDE_YAML,
      }
    end
